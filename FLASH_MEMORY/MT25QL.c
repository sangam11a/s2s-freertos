...
void Page_Write(SPI_HandleTypeDef *SPI, uint32_t address,
	...
	while (size > PAGE_SIZE) {
		...
		HAL_SPI_Transmit(SPI, (uint8_t*)data, PAGE_SIZE, 200);
		FM_Disable(SPI);
		HAL_Delay(5);
		size = size - PAGE_SIZE;
		data = data + 256;
	...
	HAL_SPI_Transmit(SPI, &cmd, 1, 100);
	HAL_SPI_Transmit(SPI, command, 3, 200);
	HAL_SPI_Transmit(SPI, (uint8_t*)data, size, 200);
	FM_Disable(SPI);
	HAL_Delay(5);
	return;
}
...
/*
 ...
 uint8_t cmd = SUBSECTOR_ERASE_32KB;
 uint8_t addr[3];
...
/*
 * @brief	similar function as above but 4 byte address mode for the FM
 * 			Parameters are same as for Subsector_Erase_32KB
 */

/*
 void Subsector_Erase_32KB_4B(uint32_t address) {
 uint8_t cmd = SUBSECTOR_ERASE_32KB_4_BYTE;
 uint8_t addr[4];
 addr[0] = (uint8_t) (address >> 24);
 addr[1] = (uint8_t) (address >> 16);
 addr[2] = (uint8_t) (address >> 8);
 ...
 FM_Enable();
 HAL_SPI_Transmit(&hspi2, &cmd, 1, 100);
 HAL_SPI_Transmit(&hspi2, addr, 4, 100);
 FM_Disable();
 HAL_Delay(1000);
 }

 void Subsector_Erase_4KB(uint32_t address) {
 uint8_t cmd = SUBSECTOR_ERASE_4KB;
 uint8_t addr[3];
 addr[0] = (uint8_t) (address >> 16);
 addr[1] = (uint8_t) (address >> 8);
 addr[2] = (uint8_t) (address);
...
/*
 * @brief	similar function as above but 4 byte address mode for the FM
 * 			Parameters are same as for Subsector_Erase_4KB
 */
/*
 void Subsector_Erase_4KB_4B(uint32_t address) {
 uint8_t cmd = SUBSECTOR_ERASE_4KB_4_BYTE;
 uint8_t addr[4];
 addr[0] = (uint8_t) (address >> 24);
 addr[1] = (uint8_t) (address >> 16);
 addr[2] = (uint8_t) (address >> 8);
...
/*
 * @brief	similar function as Page_Write but 4 byte address mode for the FM
 * 			Parameters are same as for Page_Write
 */
/*
 void Page_Write_4B(uint32_t address, uint8_t *data, uint16_t size) {
 uint8_t cmd = BYTE_4_PAGE_PROGRAM;
 uint8_t command[4];
 command[0] = (uint8_t) (address >> 24 & 0xff);
 command[1] = (uint8_t) (address >> 16 & 0xff);
