 ...
 */
void ADC_Enable() {
	HAL_GPIO_WritePin(GPIOE, ADC_CS_Pin, GPIO_PIN_RESET);
	HAL_Delay(1);
}
 ...
 */
void ADC1_Enable() {
	HAL_GPIO_WritePin(GPIOE, ADC1_CS_Pin, GPIO_PIN_RESET);
	HAL_Delay(1);
}
 ...
 */
void ADC_Disable() {
	HAL_GPIO_WritePin(GPIOE, ADC_CS_Pin, GPIO_PIN_SET);
	HAL_Delay(1);
}
 ...
 */
void ADC1_Disable() {
	HAL_GPIO_WritePin(GPIOE, ADC1_CS_Pin, GPIO_PIN_SET);
	HAL_Delay(1);
}
...
void ADC_Operate(uint16_t *adc_channels, uint16_t *adc1_channels) {
	...
	for (int x = 0; x < E_ADC_1_DATA_LEN; x++) {
		if (x < E_ADC_0_DATA_LEN) {
			AUTO_2_Select_ADC(ADC_READ);			// Data read cycle of ADC 0

		}
		AUTO_2_Select_ADC1(ADC_READ);				// Data  Read cycle from ADC 1
